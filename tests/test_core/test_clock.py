from __future__ import annotations

from datetime import date, datetime, timezone

import daily_driver.core.clock as clock_mod
from daily_driver.core.clock import iso_week, month_bounds, now, today


def test_now_returns_aware_datetime() -> None:
    result = now()
    assert result.tzinfo is not None


def test_today_returns_date() -> None:
    result = today()
    assert isinstance(result, date)
    assert not isinstance(result, datetime)


def test_frozen_time_now() -> None:
    frozen = datetime(2026, 1, 5, 12, 0, 0, tzinfo=timezone.utc)
    clock_mod.FROZEN_TIME = frozen
    try:
        assert now() == frozen
        assert today() == frozen.date()
    finally:
        clock_mod.FROZEN_TIME = None


def test_frozen_time_restored() -> None:
    clock_mod.FROZEN_TIME = None
    result = now()
    assert result.tzinfo is not None


def test_iso_week_known_date() -> None:
    # 2026-01-05 is Monday of ISO week 2 of year 2026
    d = date(2026, 1, 5)
    assert iso_week(d) == (2026, 2)


def test_iso_week_year_boundary() -> None:
    # 2024-12-30 belongs to ISO week 1 of 2025
    d = date(2024, 12, 30)
    year, week = iso_week(d)
    assert year == 2025
    assert week == 1


def test_month_bounds_regular() -> None:
    first, last = month_bounds(date(2026, 3, 15))
    assert first == date(2026, 3, 1)
    assert last == date(2026, 3, 31)


def test_month_bounds_february_leap() -> None:
    first, last = month_bounds(date(2024, 2, 10))
    assert first == date(2024, 2, 1)
    assert last == date(2024, 2, 29)


def test_month_bounds_february_non_leap() -> None:
    first, last = month_bounds(date(2026, 2, 1))
    assert first == date(2026, 2, 1)
    assert last == date(2026, 2, 28)
