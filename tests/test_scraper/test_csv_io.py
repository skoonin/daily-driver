...
def test_backfill_uses_config_budget_not_maxsize(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "daily_driver.scraper.enrichment.enrich_company_descriptions",
            side_effect=_capture_company,
        ),
        patch(
            "daily_driver.scraper.enrichment.enrich_fit_and_notes",
            side_effect=_capture_fit,
        ),
        patch("daily_driver.scraper.runner.validate_config"),
    ):
...
def test_backfill_skips_enrichment_when_all_rows_filled(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("daily_driver.scraper.enrichment.enrich_fit_and_notes") as mock_fit,
        patch("daily_driver.scraper.runner.validate_config"),
    ):
        backfill(_MINIMAL_CONFIG, csv_path)
