...
- **One status vocabulary across the tracker and `jobs.csv`**: status spelling is now normalized everywhere (case-folded, underscores and spaces turned into hyphens), so `ruled_out`, `Ruled Out`, and `ruled-out` are the same status. The `jobs.csv` `Status` column shares the tracker's recommended-status machinery — values outside the job-search set (`found`, `skipped`, `applied`, `interviewing`, `rejected`, `dropped`) now log one WARNING per run, and `prune`/backfill/enrichment canonicalize the spelling of any row they already rewrite (reads never rewrite the file, and meaning is never changed). The new `tracker.extra_statuses` config list extends the tracker's recommended set without warnings. (#TBD)
