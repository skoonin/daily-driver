...
- **Ollama enrichment now adapts its concurrency to the server's real parallelism.** When `ai.ollama.max_parallel` is set wider than the server's `OLLAMA_NUM_PARALLEL`, requests queued server-side and the queue wait counted against the per-call timeout, so a too-wide fan-out could spend a whole run on guaranteed timeouts. The client now measures the server-side queue wait per call (from the response's `total_duration`) and narrows its own concurrency toward the server's true parallelism — and a timed-out call halves the pool and retries once. No new config: it derives everything from the existing `ai.ollama.max_parallel` and the enrichment timeout. Setting `OLLAMA_NUM_PARALLEL` server-side is still the way to get more real parallel throughput. (0f4b5d8)
