...
- **A second concurrent `jobs run` / `backfill` / `prune` no longer hangs silently**: when one jobs command is already writing the workspace, a second one now prints a visible `Another jobs command is writing the workspace; waiting for it to finish...` notice the moment it has to wait, instead of parking with zero output (which read as frozen). The top-level commands wait up to 10 minutes and then give up with a clear message rather than blocking forever; an uncontended run prints no notice. `--json` stdout stays clean (the notice goes to stderr).
