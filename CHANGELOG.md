...
- **Optional LinkedIn description backfill fills Notes for LinkedIn rows**: LinkedIn's anonymous job pages emit no structured data, so the generic detail-fetch phase skips them and most LinkedIn rows keep an empty description — which makes the fit/notes pass leave Notes blank by design. A new opt-in step (`plugins.job_search.enrichment.fetch_linkedin_descriptions: true`, default off) fetches the login-free `linkedin.com/jobs/view/<id>` page during enrichment and fills the missing description so the fit/notes pass can write Notes in the same wave. It is bounded to the same rows fit/notes will enrich (`max_enrich_fit`), fill-missing-only (rows that already have a description are never re-fetched), politely throttled, and degrades gracefully: a signup-wall redirect or parse miss leaves the row untouched, and fetching stops after a run of consecutive failures. Coverage is good-but-partial by nature, since anonymous fetches are often walled.
