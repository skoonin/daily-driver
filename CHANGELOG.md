...
### Fixed
- `gathers/calendar.py`: improved error messages when icalBuddy fails -- the missing-binary warning now includes install + permission instructions, and a non-zero exit hints at plist/permissions issues based on stderr keywords. New "Calendar (icalBuddy) setup" section in `docs/developer.md` walks through binary install, Calendar access, and a minimal plist. (#38)
- `gather git`: when run without `--repo`, now consults `gather.git.search_paths` from `.dd-config.yaml` and walks each root at `maxdepth=4` to discover repositories. Commits are aggregated across all discovered repos. New module `core/git_discovery.py` houses the walker; new `GatherConfig` / `GatherGitConfig` Pydantic models add the `gather.git.search_paths` key. Default behavior unchanged when key is unset (uses `cwd`). (#40)
