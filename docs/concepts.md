...
The workspace is a plain directory. All state is in files you can read,
edit, and back up with `git` or any tool you like. Daily Driver does not
phone home and has no server component.
...
**The program owns the record; Claude owns the conversation.** Claude is
not a reliable data store. The tracker, jobs CSV, plans, and notes live
in files. Claude reads them — it does not write them except via
explicit CLI commands (`tracker update`, `jobs run`, etc.).

**One workspace per machine, typically.** The workspace discovery walks
upward from `$CWD` to find `.dd-config.yaml`. You can pass
`-w PATH` to any command to override.

**Managed vs. user territory.** Files under `.claude/*/daily-driver/`
are regenerated on `init` and on version drift. Your own commands and
agents go at the top level (`.claude/commands/*.md`) where they are
never touched. See the [Customization](configuration.md#customization)
section of `configuration.md` for the full ownership table.

**No automatic migrations.** Config schema is validated by Pydantic with
`extra="forbid"`. When a release adds or renames a key, `doctor` will
report an error and the CHANGELOG will describe the manual edit needed.
