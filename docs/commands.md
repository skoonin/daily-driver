...
- **Stale boards are re-probed.** A board that dies after it was matched would otherwise stay in the scrape list forever, because only never-probed slugs were candidates. Re-probing on an age threshold retires it into the dead cache during a normal sweep, so `--full` is no longer needed for routine cleanup. At most `discovery.max_reprobe_per_sweep` boards are re-probed per sweep, oldest first, so retirement spreads over successive sweeps instead of coming due all at once.
