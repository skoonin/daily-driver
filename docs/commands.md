...
- **An enabled board source with nothing to scrape logs a warning and moves on**, rather than failing the run: a Greenhouse/Ashby/Lever source with no boards pinned in config and an empty (or never-run) `discover-boards` cache scrapes zero boards for that platform this run. `daily-driver doctor` reports the same gap as a WARNING (`Job boards`) with a `daily-driver jobs discover-boards` fix hint.
...
- **Dormancy takes several cycles to show up.** The streak only advances when a board is actually re-probed, so a board needs `dormant_after_empty_sweeps` re-probes before it can go dormant — and re-probes are capped at `discovery.max_reprobe_per_sweep` per platform per sweep. On a large workspace that is weeks to months, during which the `Dormant` column correctly shows nothing. An empty column means "not armed yet", not "not working"; `--full` arms boards faster because it re-probes everything at once.
- **The sweep prints one row per platform**: `Probed`, `Matched`, `New`, `Empty`, `Dormant`, `Dead`, `Failed`. `Matched`, `Empty` and `Dormant` are running cache totals; the rest count only this sweep, and the two that are additions to the cache (`New`, `Dead`) carry a `+`. A dash means zero, so the counts that matter stand out. Anything needing action — probes that failed, or a platform swept against a cached slug list because the upstream fetch was unreachable — is called out underneath. `--json` carries every field, including the per-platform `restaled` count the table omits.
...
A missing `jobs.csv` only fails (exit 1) once a run history exists — a workspace that ran before and then lost the file would otherwise report zero counts indistinguishable from a workspace that legitimately has no jobs. A workspace that has never scraped has no `jobs.csv` by definition, so `jobs status` reports zero counts and exits 0 there instead.
...
A missing `jobs.csv` fails outright (exit 1) instead of reporting nothing due for verification — a workspace pointing at the wrong path would otherwise look identical to one with nothing to check.
...
A missing `jobs.csv` fails outright (exit 1) instead of reporting no rows match prune criteria — a workspace pointing at the wrong path would otherwise look identical to one with nothing to prune.
