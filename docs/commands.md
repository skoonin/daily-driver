...
- `-j` / `--json` — emit the completion summary (`rows`, `skipped`, `needs_before`, `needs_after`, `enriched`, `comp_filled`, `comp_repaired`, `descriptions_filled`, `elapsed_seconds`; `dry_run` adds `fit_cap`, `comp_needs`, and `description_needs`) in the `{schema, data}` envelope, with the live progress block suppressed.
