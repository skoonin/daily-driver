...
On a TTY, `jobs run` shows a live, in-place progress display: a "Scraping sources" group (one row per enabled source with a status marker — `>` running, `-` done, `x` failed — and a per-source count) and an "Enriching jobs" group. It ends with a `Completed:` summary line reconciling totals (found, new, matched location, plus `(N skipped by location)` when rows were dropped for location). The live block renders on any TTY regardless of verbosity; verbosity controls only how much scrolls above it — warnings at normal, INFO heartbeats and per-source timings at `-v`, the full stream at `-vv`. Problems surface live above the block as they happen, with a terse `Warnings: N` count at the end. Non-interactive output (cron, launchd, pipes) falls back to plain lines with no live display. During the serial Apple scrape phase the browser runs headless whenever the live block is active.
