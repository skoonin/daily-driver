...
**Use JobSpy** when the source is supported (LinkedIn, Indeed, ZipRecruiter, Glassdoor, Google Jobs) and you don't need custom field extraction. Tradeoff: opaque internals break when upstream changes — Google for Jobs is currently broken upstream (token/JS-gated; returns 0) and is *not* shipped. The library is a developer-facing implementation detail: it surfaces to users only as the two site-named sources `linkedin` and `indeed`. Each registers in `SCRAPERS` as its own site (the `jobs run -S linkedin` / `-S indeed` selectors), both backed by `scrape_jobspy`. The runner (`runner._jobspy_scrape_plan`) collapses the two into one merged `scrape_jobs(site_name=[...])` call when both are enabled with equal query knobs (`results_wanted_per_query` / `hours_old`), and splits them into separate calls otherwise. Per-row Source attribution survives because `normalize_jobspy_row` stamps each row from JobSpy's own `site` field, so even a merged call yields rows labeled `linkedin` / `indeed` individually. In `.dd-config.yaml` they are top-level source keys (`sources.linkedin`, `sources.indeed`); `country` (Indeed's `country_indeed`) lives on the `indeed` toggle only.
