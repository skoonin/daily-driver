...
- `live_log_window(active)` is a context manager spanning a run that owns the terminal with a live progress display. Records stream live: Rich relocates each log write above the active region (thread-safe, via the live render lock), so warnings surface as they happen instead of being held to the end. The window tallies WARN+ records and closes with a terse `Warnings: N (shown above)` reconciliation line in normal mode (no timestamps). A no-op when inactive or before `configure()`. The live block renders on any TTY regardless of verbosity; `-v`/`-vv` only raise how much (INFO/DEBUG) scrolls above it.
