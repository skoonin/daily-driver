...
- **Enrichment as in-place flush.** Enrichers replace row slots in `_JobSink.rows`; `_JobSink.flush()` snapshots `rows` AND rewrites the whole file through `csv_io.atomic_write_rows` under one held `_rows_lock` + `jobs.lock`, carrying unknown / hand-added columns through POSITIONALLY via a parallel `row_extras` list (aligned 1:1 with `rows`, never keyed by Link — so every row keeps exactly its own extras even with a blank or duplicate Link), so the whole flush is atomic relative to a concurrent append — no interleave can duplicate or drop rows. The lock is held only for the rewrite, never across LLM calls. `flush_periodic()` is the in-loop hook (after each phase and every ~25 LLM results): an `OSError` there is a distinct **PERSISTENCE failure** — it sets the sticky `persistence_degraded` flag and logs once, but enrichment continues (memory state intact). At run end `run()` warns if degraded and the FINAL `flush()` propagates its failure loudly (after the completion manifest is written), so a silent disk problem can't masquerade as a healthy run. `jobs backfill` (the dedicated subcommand) shares this same `_JobSink` + `_enrich_wave` driver to resume empty rows; it reads jobs.csv and rewrites it ALL inside one `jobs.lock` window held for the whole read → enrich → rewrite (it rewrites from a snapshot rather than appending, so it cannot drop the lock mid-enrichment) and builds the sink with `external_lock_held=True` so the per-flush sections don't re-take the non-reentrant flock. backfill also sets `skip_flush_if_unchanged` (no rewrite, and a lazy `pre_write_hook` backup, only when content actually changed) and mirrors `run()`'s degraded-save warning + names the backup on a final-flush `OSError`.
