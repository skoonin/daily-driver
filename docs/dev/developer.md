...
- **Enrichment as in-place flush.** Enrichers replace row slots in `_JobSink.rows`; `_JobSink.flush()` snapshots `rows` AND rewrites the whole file through the one backfill rewrite path (`csv_io.atomic_write_rows` + the unknown-column carry-through) under one held `_rows_lock` + `jobs.lock`, so the whole flush is atomic relative to a concurrent append — no interleave can duplicate or drop rows. The lock is held only for the rewrite, never across LLM calls. `flush_periodic()` is the in-loop hook (after each phase and every ~25 LLM results): an `OSError` there is a distinct **PERSISTENCE failure** — it sets the sticky `persistence_degraded` flag and logs once, but enrichment continues (memory state intact). At run end `run()` warns if degraded and the FINAL `flush()` propagates its failure loudly (after the completion manifest is written), so a silent disk problem can't masquerade as a healthy run. `jobs run --backfill` resumes empty rows.
